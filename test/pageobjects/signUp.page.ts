...
class SignUpPage {
  ...
  private locators = {
    chooseYourRoleHeading: '//android.widget.TextView[@text="Choose Your Role"]',
    senderRoleButton: '//android.widget.TextView[@text="Send packages to any destination."]',
    courierRoleButton:
    ...
    phoneNumberInputField: '//android.widget.EditText[@text="Enter Phone Number"]',
    ...
    otpVerificationHeading: '//android.widget.TextView[@text="OTP Verification"]',
  ...
  async selectRole(role: 'sender' | 'courier'): Promise<void> {
    const roleButton =
      role === 'sender' ? $(this.locators.senderRoleButton) : $(this.locators.courierRoleButton);

    await waitAndClick(roleButton, `${role} role button`);
  }

  async waitForSenderOnboardingScreen(): Promise<void> {
    await waitForElementToAppear($(this.locators.senderFormHeading), 'Sender onboarding heading');
  }

  async inputFirstName(firstName: string): Promise<void> {
    await waitAndInput($(this.locators.firstNameInputField), firstName, 'First Name input field');
  }

  async inputLastName(lastName: string): Promise<void> {
    await waitAndInput($(this.locators.lastNameInputField), lastName, 'Last Name input field');
  }
  ...
  async selectModeOfIdentification(mode: 'NIN' | 'BVN'): Promise<void> {
    ...
    const option = mode === 'NIN' ? $(this.locators.NINOption) : $(this.locators.BVNOption);
  ...
  async inputEmail(email: string): Promise<void> {
    await waitAndInput($(this.locators.emailInputField), email, 'Email input field');
  }

  async inputPassword(password: string): Promise<void> {
    await waitAndInput($(this.locators.passwordInputField), password, 'Password input field');
  }
  ...
  async completeSenderSignUp(modeOfIdentification: 'NIN' | 'BVN'): Promise<void> {
