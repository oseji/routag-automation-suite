...
class DeliveryRoutePage {
  private locators = {
    deliveryRoutePageHeading: 'android=new UiSelector().text("Set Delivery Route")',
    ...
    pickupLocationInputField: 'android=new UiSelector().text("Enter source address")',
    destinationLocationInputField: 'android=new UiSelector().text("Enter destination address")',
  ...
  async clickClearPickupLocationButton(): Promise<void> {
    await waitAndClick($(this.locators.clearPickupLocationButton), 'Clear pickup location button');
  }
  ...
  async selectPickupLocation(): Promise<void> {
    await waitAndClick($(this.locators.pickupLocationSelection), 'Pickup location selection');
  }
  ...
  async fillDeliveryRouteDetailsAndContinue(
    ...
    const clearDestination = $(this.locators.clearDestinationLocationButton);
