...
class DeliveryRoutePage {
  private locators = {
    deliveryDetailsFormHeading: 'android=new UiSelector().text("Delivery Details")',
    packageWeightInputField: 'android=new UiSelector().text("Max Weight 20kg")',
    packageDescriptionInputField: 'android=new UiSelector().text("Describe Package ")',
    packageClassificationDropdown: 'android=new UiSelector().text("Describe package ")',
    packageClassificationSelection: '~SHOES BAGS',
    receiversNameInputField: 'android=new UiSelector().text("Enter Receivers Name ")',
    receiversPhoneNumberInputField:
      'android=new UiSelector().text("Enter Receivers Phone Number ")',
    productValueInputField: 'android=new UiSelector().text("Product value in naira")',
    ...
    successfulImageUploadMessage: '//android.widget.TextView[@text="Image uploaded successfully"]',
  ...
  async inputProductValue(value: string): Promise<void> {
    await waitAndInput($(this.locators.productValueInputField), value, 'Product value input field');
  }

  async clickUploadProductImageButton(): Promise<void> {
    await waitAndClick($(this.locators.uploadProductImageButton), 'Upload product image button');
  }

  async clickSelectedImage(): Promise<void> {
    await waitAndClick($(this.locators.selectedImage), 'Selected image from gallery');
  }
  ...
  async clickFindCourierButton(): Promise<void> {
    await waitAndClick($(this.locators.findCourierButton), 'Find Courier button');
  }
