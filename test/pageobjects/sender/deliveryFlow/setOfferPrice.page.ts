...
class SetOfferPricePage {
  private locators = {
    ...
    decreasePriceButton: 'android=new UiSelector().description("100").instance(0)',
    increasePriceButton: 'android=new UiSelector().description("100").instance(1)',
    proceedButton: '~Proceed',
    closeButton: '~Close',
    closeLoadingLayout: 'android=new UiSelector().description("Close").instance(1)',
  };

  async waitForSetOfferPricePageToLoad(): Promise<void> {
    await waitForElementToAppear($(this.locators.setOfferPriceHeading), 'Set Offer Price heading');
  }

  async clickDecreasePriceButton(): Promise<void> {
    await waitAndClick($(this.locators.decreasePriceButton), 'Decrease price button');
  }

  async clickIncreasePriceButton(): Promise<void> {
    await waitAndClick($(this.locators.increasePriceButton), 'Increase price button');
  }
  ...
  async clickCloseLoadingLayout(): Promise<void> {
    await waitAndClick($(this.locators.closeLoadingLayout), 'Close loading layout');
  }
