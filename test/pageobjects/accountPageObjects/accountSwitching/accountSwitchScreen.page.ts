...
class accountSwitchScreen {
    private locators = {
        accountSwitchScreenHeading:
            'android=new UiSelector().text("Switch Your Role")',
        senderRoleOption:
    ...
    async switchRole(role: "Sender" | "Courier"): Promise<void> {
        if (role === "Sender") {
            await waitAndClick(
                $(this.locators.senderRoleOption),
                "Sender role option",
            );
        } else if (role === "Courier") {
            await waitAndClick(
                $(this.locators.courierRoleOption),
                "Courier role option",
            );
        }
    }

    async completeAccountSwitchingFlow(
        role: "Sender" | "Courier",
    ): Promise<void> {
