...
class CreateProfileScreen {
  private locators = {
    createProfileScreenHeading: 'android=new UiSelector().text("Create New Profile")',
    meansOfTravelDropdown: 'android=new UiSelector().text("Select Your Travel Means")',
    publicTransportOption: 'android=new UiSelector().text("Public Transportation")',
    privateVehicleOption: 'android=new UiSelector().text("Private Vehicle")',
    uploadutilityBillButton: 'android=new UiSelector().text("Select an image")',
    utilityBillImage:
  ...
  async selectMeansOfTravel(
    ...
    await waitAndClick($(this.locators.meansOfTravelDropdown), 'Means of travel dropdown');

    if (meansOfTravel === 'Public Transportation') {
      await waitAndClick($(this.locators.publicTransportOption), 'Public transportation option');
    } else if (meansOfTravel === 'Private Vehicle') {
      await waitAndClick($(this.locators.privateVehicleOption), 'Private vehicle option');
    }
  }

  async uploadUtilityBill(): Promise<void> {
    await waitAndClick($(this.locators.uploadutilityBillButton), 'Upload utility bill button');
    await waitAndClick($(this.locators.utilityBillImage), 'Utility bill image');
  }
