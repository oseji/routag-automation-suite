...
class accountPage {
  ...
  private accountSwitchingLocators = {
    tryingToSwitchModalHeading: 'android=new UiSelector().text("Trying to switch account?")',
  ...
  async clickPersonalInfoButton(): Promise<void> {
    await waitAndClick($(this.locators.personalInfoButton), 'Personal Info button');
  }

  async clickVehicleInfoButton(): Promise<void> {
    await waitAndClick($(this.locators.vehicleInfoButton), 'Vehicle Info button');
  }
  ...
  async clickDeleteAccountButton(): Promise<void> {
    await waitAndClick($(this.locators.deleteAccountButton), 'Delete Account button');
  }

  // account switching actions
  async clickSwitchAccountButton(): Promise<void> {
    await waitAndClick($(this.locators.switchAccountButton), 'Switch Account button');
  }

  async waitForTryingToSwitchModalToAppear(role: 'Sender' | 'Courier'): Promise<void> {
    ...
    if (role === 'Sender') {
      await waitForElementToAppear(
        $(this.accountSwitchingLocators.tryingToSwitchToSenderProfileModalMessage),
        'Trying to switch to sender profile modal message'
      );
    } else if (role === 'Courier') {
      await waitForElementToAppear(
        $(this.accountSwitchingLocators.tryingToSwitchToCourierProfileModalMessage),
  ...
  private async tryWaitForTryingToSwitchModalToAppear(
    ...
    try {
      await $(this.accountSwitchingLocators.tryingToSwitchModalHeading).waitForDisplayed({
        timeout: 5000,
      });
      if (role === 'Sender') {
        await waitForElementToAppear(
  ...
  async checkTryingToSwitchModal(role: 'Sender' | 'Courier'): Promise<void> {
    const modalAppeared = await this.tryWaitForTryingToSwitchModalToAppear(role);
